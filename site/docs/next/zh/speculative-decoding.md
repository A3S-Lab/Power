...
最新精确构建的 9 次采集稳态中位数为 174.413 token/s，最低 172.723，最高 177.150；只有 4/9 个样本达到 175，因此不能视为稳定 175 token/s 下限。较安静主机的历史高水位为 176.6109 token/s；同一制品全词表 K7/S7 对照为 147.0207 token/s。

历史短任务校准使用固定 K6/S6/B8，在 12 题、256-token 配对采集中达到 46.923 token/s；关闭推测执行为 28.713 token/s，提升 63.42%。两种模式的 12 个最终答案和 9/12 分数相同，接受率为 26.81%，每次目标前向提交 2.591 个已验证 token，回放为零。
