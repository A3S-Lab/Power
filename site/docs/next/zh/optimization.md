...
| 前缀复用 | 显式缓存 key、认证命名空间、有界 KV 生命周期、命中／token 指标 | 运行时 + 后端 |
...
托管 API 还接受显式 `prompt_cache_key`。Power 校验该字段，按认证身份、端点与模型做哈希隔离，只转发给声明支持精确前缀复用的后端。当前 llama.cpp 文本请求使用每模型有界 LRU／TTL 上下文映射；无法证明的循环状态回滚会记为 miss，其他路径失败关闭。原生 MTP 与缓存会话尚不能组合，完整约束见[规范缓存文档](https://github.com/A3S-Lab/Power/blob/main/docs/prompt-prefix-cache.md)。
