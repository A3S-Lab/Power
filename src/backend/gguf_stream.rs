...
#[cfg(feature = "picolm")]
mod architecture;
#[cfg(all(feature = "picolm", test))]
pub(crate) use architecture::Qwen35ArchitectureArgs;
#[cfg(feature = "picolm")]
pub use architecture::{GgufArchitecture, Qwen35Architecture, Qwen35LayerKind};
...
impl GgufFile {
    ...
    fn from_storage(storage: GgufStorage) -> Result<Self> {
        ...
        model.validate_tensor_bounds()?;
        Ok(model)
    }

    /// Validate every descriptor against the immutable backing bytes without
    /// faulting tensor data into memory.
    pub fn validate_tensor_bounds(&self) -> Result<()> {
        for (name, descriptor) in &self.meta.tensors {
            checked_tensor_byte_range(
                name,
                self.meta.tensor_data_offset,
                self.data_len,
                descriptor,
            )?;
        }
...
fn parse_gguf_header(data: *const u8, len: usize) -> Result<GgufMeta> {
    ...
    let arch_name = optional_string_metadata(&kv, "general.architecture", "llama")?;

    let arch_prefix = arch_name.as_str();
    ...
    let architecture = architecture::parse(&arch_name, &kv, n_layers)?;
...
impl MetaValue {
    ...
    fn as_bool_array(&self) -> Option<Vec<bool>> {
        match self {
