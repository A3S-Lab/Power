...
__device__ __forceinline__ float depthwise_accumulate(
  ...
  for (uint64_t kernel_y = 0; kernel_y < kernel_height; ++kernel_y) {
    const uint64_t input_y = output_y * stride_height + kernel_y * dilation;
    for (uint64_t kernel_x = 0; kernel_x < kernel_width; ++kernel_x) {
      const uint64_t input_x = output_x * stride_width + kernel_x * dilation;
      ...
      const float product = __fmul_rn(input[input_index], kernel[kernel_index]);
      if (first) {
        accumulator = product;
        first = false;
      } else {
...
extern "C" __global__ void depthwise_conv2d_f32(
  ...
  output[index] = depthwise_accumulate(
      ...
      kernel_width, stride_height, stride_width, dilation, input_offset,
      input_stride_batch, input_stride_channel, input_stride_height,
      input_stride_width, kernel_offset, kernel_stride_channel,
