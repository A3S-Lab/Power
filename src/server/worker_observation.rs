...
mod tests {
    ...
    #[test]
    fn pending_profile_bound_ready_health_never_advertises_without_accepts_work() {
        // Gap: pending unlocks executor Ready / accepts_work, but backend-owned
        // may_advertise stays false → runtime accepts_work false → empty
        // ready_phases. Must not advertise P/D from Ready health alone.
        use crate::serving::{
            BackendOwnedPhaseExecutor, ServingCompositionPhaseExecution,
            ServingCompositionPhaseExecutor, ServingCompositionStateOwnership,
            ServingCompositionTransport,
        };
        ...
        let profile = ServingExecutionProfile::prefill_decode(PrefillDecodeExecutionProfile {
            ...
            phase_executor: Some(ServingCompositionPhaseExecutor::BackendOwned),
            state_ownership: Some(ServingCompositionStateOwnership::ProfileBound),
            phase_execution: Some(ServingCompositionPhaseExecution::Pending),
        })
        .unwrap();
