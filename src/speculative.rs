...
//! head, adaptive verification length, and lossless target verification. These
//! primitives provide the model-neutral runtime seam for that architecture.
//! [`NgramContextDrafter`] is only a zero-weight integration baseline; it is
//! not a trained DSpark drafter. A trained model adapter can implement
//! [`Drafter`] without adding model names or tensor layouts to Power.
//!
//! # State Rollback
//!
