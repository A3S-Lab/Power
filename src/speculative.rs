...
/// length, never permits a proposal wider than the backend's rollback window,
/// and opens a one-way target-only circuit after a sustained low-yield window.
...
impl AdaptiveSpeculationController {
    ...
    /// Create a controller whose effective maximum is rollback-safe.
    pub fn new(initial: usize, min: usize, max: usize, rollback_limit: usize) -> Self {
        let max = max.max(1).min(rollback_limit.max(1));
...
mod tests {
    ...
    #[test]
    fn adaptive_speculation_never_exceeds_rollback_window() {
