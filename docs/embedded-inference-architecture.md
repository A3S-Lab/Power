...
This design borrows systems ideas from Colibri at pinned revision
`b085b48888a88d9a1c00b151a9979774b72cdbfd`, while retaining Power's TEE,
encrypted-model, signature, attestation, resource-bound, and receipt features.
No Colibri source code is copied into Power.
...
- `start_prefetch` starts bounded blocking I/O immediately. A model can prefetch
  layer N+1, compute layer N, then await the task, providing the one-layer-ahead
...
The current foundation does not yet implement Colibri's verified multi-drive
mirrors, automatic hardware placement planner, encrypted persistent model
state, or a cross-model benchmark runner. These should extend the same runtime
